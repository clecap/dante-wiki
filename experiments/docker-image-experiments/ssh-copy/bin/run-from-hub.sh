...
echo ""; echo "Starting container ${CONTAINER_NAME} for docker image ${HUB_IMAGE_NAME}..."
docker run -d --name ${CONTAINER_NAME} --network bridge -p:${PORT}:22 --env USERNAME=${USERNAME} ${HUB_IMAGE_NAME} 
echo "DONE"

echo ""; echo "*** Copying in the public login key"
docker cp ${DIR}/../login-key.pub ${CONTAINER_NAME}:/login-key.pub
